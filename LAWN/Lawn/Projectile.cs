	...
	internal class Projectile : GameObject
	{
		...
		public void DoImpact(Zombie theZombie)
		{
			...
			else if (theZombie != null)
			{
				...
				ProjectileType mProjectileType2 = mProjectileType;
				int num7 = 6;
		...
		public void UpdateMotion()
		{
			...
			int mRow = base.mRow;
			float posYBasedOnRow = mBoard.GetPosYBasedOnRow(mPosX, base.mRow);
			if (mMotionType == ProjectileMotion.MOTION_LOBBED)
			{
				UpdateLobMotion();
			}
			...
			float posYBasedOnRow2 = mBoard.GetPosYBasedOnRow(mPosX, mRow);
		...
		public void UpdateNormalMotion()
		{
			...
			else if (mMotionType == ProjectileMotion.MOTION_STAR)
			{
				...
				if (mVelY != 0f)
				{
					int num = mRow = mBoard.PixelToGridYKeepOnBoard((int)mPosX, (int)mPosY);
				}
			}
			...
			else if (mMotionType == ProjectileMotion.MOTION_FLOAT_OVER)
			{
				if (mVelZ < 0f)
				{
					...
					mPosY += 3f * mVelZ;
					mRotation = 0.3f + -0.700000048f * mVelZ * (float)Math.PI * 0.25f;
				}
				mPosX += 1.2f;
			}
