...
namespace Lawn
{
	internal class Challenge : StoreListener
	{
		...
		private int[] aDoubleChance = new int[6]
		{
			0,
			30,
			10,
		...
		private int[] aTripleChance = new int[6]
		{
			0,
			0,
			0,
		...
		private int[] aPailChance = new int[6]
		{
			0,
			0,
			0,
		...
		private int[] aConeChance = new int[6]
		{
			0,
			0,
			30,
		...
		public bool MouseMove(int x, int y)
		{
			if (mApp.mGameMode == GameMode.GAMEMODE_CHALLENGE_BEGHOULED && !mBoard.HasLevelAwardDropped())
			{
				...
				mBoard.MouseHitTest(x, y, out theHitResult, false);
		...
		public void BeghouledFallIntoSquare(int x, int y, BeghouledBoardState theBoardState)
		{
			...
			while (true)
			{
				if (num >= 0)
				{
					topPlantAt = mBoard.GetTopPlantAt(x, num, PlantPriority.TOPPLANT_ANY);
					if (topPlantAt != null)
					{
						break;
					}
		...
		public void DrawBackdrop(Graphics g)
		{
			...
			{
				int mMainCounter = mBoard.mMainCounter;
				int num = 3000;
			}
			if (mApp.mGameMode == GameMode.GAMEMODE_CHALLENGE_ZEN_GARDEN)
			{
		...
		public void InitZombieWaves()
		{
			...
			{
				ZombieType[] array4 = new ZombieType[1]
				{
			...
			{
				ZombieType[] array14 = new ZombieType[1]
				{
					ZombieType.ZOMBIE_BALLOON
				};
